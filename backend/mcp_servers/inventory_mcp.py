from backend.db.client import get_part, check_compatibility, search_by_model, get_part_by_mpn
...
def _get_by_mpn(mpn: str) -> dict:
    ...
    if not results:
        return {"error": f"No parts found with MPN {mpn}. Try searching by model number instead."}
